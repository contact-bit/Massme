...
export function createReviewToken(params: {
  ...
  if (!secret) {
    throw new Error(
      "Missing env REVIEW_TOKEN_SECRET"
    );
  }
...
export function verifyReviewToken(
  ...
  try {
    ...
    try {
      payload = JSON.parse(
        unb64urlToString(body)
      ) as Payload;
    } catch {
      return {
        ok: false as const,
        reason: "bad_payload" as const,
      };
    }
    ...
    if (
      ...
      return {
        ok: false as const,
        reason: "order_mismatch" as const,
      };
    }
