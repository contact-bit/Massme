...
// ✅ IMPORTANT: remettre le padding "=" avant de décoder
...
export function createReviewToken(params: { orderId: string; email: string; ttlDays?: number }) {
  ...
  const payload: Payload = { orderId: params.orderId, email: params.email, exp };
  const body = b64urlJson(payload);
  const sig = sign(body, secret);
  return `${body}.${sig}`;
}

export function verifyReviewToken(token: string, expected: { orderId: string; email: string }) {
  const secret = process.env.REVIEW_TOKEN_SECRET;
  if (!secret) throw new Error("Missing env REVIEW_TOKEN_SECRET");

  const [body, sig] = token.split(".");
  if (!body || !sig) return { ok: false as const, reason: "bad_format" as const };
